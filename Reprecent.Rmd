...
```{r}
Unemployment_rate_melt_dt <- plot_d[order(plot_d$date),] %>% as.data.frame()
tmp <- c()
for(i in 1:nrow(Unemployment_rate_melt_dt)-11){
  tmp <- c(tmp,mean(Unemployment_rate_melt_dt$value[i:(i+11)]))
