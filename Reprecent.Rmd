---
title: "Dynamic Default Rates fit_feature"
author: "Naoya Hieda"
...
## Reprecentの内容を論文のデータよりも先のデータに対しても当てはめている
...
repro_data <- charge_off_data[7:134,c(5,9,10,8,4,3)] %>% apply(2,as.numeric)
...
Regression_AR <- {}
for(i in 1:6){
tmp <- data.frame(y = pnorm_co_data[2:128,i],x = pnorm_co_data[1:127,i])
fit <- lm(y ~ x ,data=tmp)
...
for(i in c(1:6)){
  f <- function(q)
    -sum(log(dnorm((
      pnorm_co_data[2:128,i]-
        sqrt(beta_[i])*pnorm_co_data[1:127,i]-
        ((1-sqrt(beta_[i]))/sqrt(1-factor_correlation[i]))*qnorm(q))
      )))
  optim_result <- optim(par = 0+1e-2,method = 'Brent',f,lower = 0+1e-2, upper =1-1e-2,hessian = TRUE)
...
Inovations <- t((t(X_t_auto[2:128,])-sqrt(beta_)*t(X_t_auto[1:127,]))/sqrt(1-beta_))
...
その前に、論文のAppendixを参考に相関行列の固有値に対して仮説検定を行う。  
...
考えたデフォルト分布がどれだけ条件付きの状態でデフォルト確率を表現できているか、確認していく。  
観測されている因子の合計と観測されていない因子を用いて、潜在変数$Z_{it}$を示す
$$
