...
tmp_diff<-diff(Unem_normalise$normalised_value,differences = 3)/3
tmp <- Unem_normalise$normalised_value[1:length(Unem_normalise$normalised_value) %% 3 == 0] + tmp_diff[1:length(Unem_normalise$normalised_value) %% 3 == 0]

tmp2 <- c()
for(i in 1:(length(tmp)-4)){
  tmp2 <- c(tmp2,tmp[i+4]-tmp[i])
}
...
for(i in 1:(length(INDPRO$INDPRO)-4)){
  tmp <- c(tmp,(INDPRO$INDPRO[i+4]-INDPRO$INDPRO[i])/INDPRO$INDPRO[i])
}
